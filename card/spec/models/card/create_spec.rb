...
describe Card, "create junction three parts" do
  ...
  it "sets left and right ids" do
    ...
    expect(sugar_milk_flour.right_id).to eq(described_class.fetch_id("Flour"))
    expect(sugar_milk.left_id).to eq(described_class.fetch_id("Sugar"))
    expect(sugar_milk.right_id).to eq(described_class.fetch_id("Milk"))
  end
end
...
describe Card, "Joe User" do
  before do
    Card::Auth.as_bot do
      ...
      described_class.create name: "Cardtype F+*type+*create", type: "Pointer", content: "[[r3]]"
    end
  ...
  it "does not have r3 permissions" do
    expect(@ucard.fetch(new: {}, trait: :roles).item_names).not_to be_member(@r3.name)
  end
